...
//                  $out_quantity_sub += $v->out_quantity_sub->quantity;
//                  $unit_sub = $v->out_quantity_sub->unit;

                  $orderItem = DeliveryOrderSettlementItem::create($goodsId);
                  $orderItem->order_id = $k;
                  $orderItem->out_quantity = $v->out_quantity;
//                  $orderItem->out_quantity_sub = $v->out_quantity_sub;
                  $orderItem->settle_quantity = $v->out_quantity;
//                  $orderItem->settle_quantity_sub = $v->out_quantity_sub;
                  $orderItem->loss_quantity = new Quantity(0, $v->out_quantity->unit);
                  /*if($v->out_quantity_sub->quantity>0)
                  $orderItem->loss_quantity_sub = new Quantity(0, $v->out_quantity_sub->unit);*/
                  $item->addOrderItem($orderItem);
                }

                $item->out_quantity = new Quantity($out_quantity, $unit);
                $item->settle_quantity = new Quantity($out_quantity, $unit);
                $item->loss_quantity = new Quantity(0, $unit);
                /*if($out_quantity_sub>0){
                  $item->out_quantity_sub = new Quantity($out_quantity_sub, $unit_sub);
                  $item->settle_quantity_sub = new Quantity($out_quantity_sub, $unit_sub);
