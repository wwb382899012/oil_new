...
$this->loadHeaderWithNewUI($menus, $buttons, '/bankFlow/');
?>
                ...
                <div class="z-card-part">
                    ...
                    <div class="flex-grid form-group">
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">银行流水编号</p>
                            <input type="text" class="form-control" id="code" name="obj[code]" placeholder="银行流水编号"
                                   data-bind="value:code">
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill ">交易主体</p>
                            <select class="form-control show-menu-arrow selectpicker" title="交易主体" id="ownership" name="obj[ownership]"
                                    data-live-search="true"
                                    data-bind="options:corporations,optionsText:'name',optionsCaption: '交易主体',value: corporation_id, optionsValue:'corporation_id',valueAllowUnset: true">
                            </select>
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">收款银行</p>
                            <select class="form-control selectpicker show-menu-arrow " title="收款银行" id="ownership" name="obj[ownership]"
                                    data-bind="selectPickerOptions:selectedAccounts,
                                    optionsText:function(item) {
                                       return item.bank_name + '--' + item.account_no
                                   },
                                   optionsCaption: '收款银行',
                                   selectpicker: account,
                                   optionsValue:'account_id',
                                   valueAllowUnset: true">
                            </select>

<!--                            <select class="form-control selectpicker show-menu-arrow" title="请选择合同信息"-->
<!--                                    data-bind="-->
...
<!--                            </select>-->
                    ...
                    <div class="flex-grid form-group">
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">银行账户名</p>
                            <input type="text" class="form-control" id="account" name="obj[account_name]"
                                   placeholder="银行账户名" data-bind="value:account_name">
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">付款公司</p>
                            <input class="form-control" data-bind="value: pay_partner"/>
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">收款时间</p>
                            <input type="text" class="form-control" id="receive_date" name="obj[receive_date]"
                                   placeholder="收款时间" data-bind="date:receive_date">
                        </label>
                    </div>

                    <div class="flex-grid form-group">
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">付款银行</p>
                            <input type="text" class="form-control" id="pay_bank" name="obj[pay_bank]"
                                   placeholder="付款银行" data-bind="value:pay_bank">
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">收款金额</p>
                            <div class="input-group">
                                <input type="text" class="form-control" id="amount" name="obj[amount]"
                                       placeholder="收款金额" data-bind="money:amount">
                                <span class="input-group-addon ellipsis" data-bind="moneyChineseText:amount, attr: {title: titleAmount}" style="max-width: 200px;"></span>
                            </div>
                        </label>
                        <label for="type" class="col col-count-3 field">
                            <p class="form-cell-title must-fill">币种</p>
                            <select class="form-control show-menu-arrow selectpicker" title="币种" id="currency" name="obj[currency]"
                                    data-bind="options:currencies,optionsText:'name',optionsCaption: '币种',value: currency, optionsValue:'id',valueAllowUnset: true">
                            </select>
                        </label>
                    </div>
                    ...
                    <div class="flex-grid form-group">
                        <label for="type" class="col col-count-3 field">
                            ...
                            <input type="text" class="form-control" id="code" name="obj[exchange_rate]" placeholder="汇率"
                                   data-bind="value:exchange_rate">
                        </label>
                    </div>
                    <div class="flex-grid form-group">
                        <label for="type" class="col col-count-1 field">
                            <p class="form-cell-title">备注</p>
                            <textarea type="text" class="form-control" id="remark" name="obj[remark]" placeholder="备注"
...
<script>
    ...
    function ViewModel(option) {
        ...
        self.titleAmount = ko.computed(function() {
            return inc.moneyToChinese(self.amount() / 100)
        })
        self.currency = ko.observable(o.currency).extend({required: true});
        self.currencies = o.currencies;
        self.receive_date = ko.observable(o.receive_date).extend({
            date: true, custom: {
                params: function (v) {
                    var today = new Date();
