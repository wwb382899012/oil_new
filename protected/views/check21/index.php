...
$form_array = array(
 ...
 'input_array' => array(
   ...
   array('type' => 'select', 'map_name'=>'contract_category_buy_type', 'key' => 'e.category', 'text' => '合同类型'),
   array('type' => 'text', 'key' => 'cf.code_out*', 'text' => '外部合同编号'),
   array('type' => 'select', 'noAll'=>'1', 'map_name'=>'stock_batch_settle_check_status', 'key' => 'checkStatus', 'text' => '审核状态'),
...
$array = array(
  ...
  array('key' => 'contract_id,contract_code', 'type' => 'href', 'style' => 'width:140px;text-align:center', 'text' => '采购合同编号', 'href_text' => '<a id="t_{1}" title="{2}" target="_blank" href="/contract/detail/?id={1}&t=1" target="_blank">{2}</a>'),
  ...
  array('key' => 'category', 'type' => 'map_val', 'map_name' => 'contract_category_buy_type', 'style' => 'width:80px;text-align:center', 'text' => '合同类型'),
...
function getRowActions($row, $self) {
	...
	if($row['checkStatus'] == 1) {
		$html = '<a href="/check21/check?id=' . $row["obj_id"] . '" title="审核">审核</a>';
	} else {
		$html = '<a href="/check21/detail?detail_id=' . $row["detail_id"] . '" title="查看">查看</a>';
	}
	return $html;
}
