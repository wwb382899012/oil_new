    ...
    */
    ...
    public static function create(DeliveryOrder $deliveryOrder)
    {
        ...
        if(is_array($deliveryOrder->items) && !empty($deliveryOrder->items)) {
            ...
            {
                $item=GoodsExpenseSettlementItem::create($g->goods_id);
                $item->relation_id = $deliveryOrder->order_id;
                $item->out_quantity = $g->out_quantity;
                $item->settle_quantity = $g->out_quantity;
                $item->loss_quantity = new Quantity(0, $g->out_quantity->unit);

                $orderItem = DeliveryOrderSettlementItem::create($g->goods_id);
                $orderItem->item_id = $item->item_id;
                $orderItem->order_id = $deliveryOrder->order_id;
                $orderItem->out_quantity = $g->out_quantity;
                $orderItem->settle_quantity = $g->out_quantity;
                $orderItem->loss_quantity = new Quantity(0, $g->out_quantity->unit);
