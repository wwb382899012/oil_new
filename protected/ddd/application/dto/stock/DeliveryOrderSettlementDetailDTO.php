...
use ddd\domain\entity\contractSettlement\DeliveryOrderSettlementDetail;
use ddd\domain\entity\value\Quantity;
use ddd\repository\GoodsRepository;

class DeliveryOrderSettlementDetailDTO extends BaseDTO
{
    public $settle_id;          //结算单id
    public $order_id;           //发货单id
//    public $settle_date;        //结算日期
    ...
    public $out_quantity;  //入库单数量
    public $out_quantity_sub;  //入库单数量
    ...
    public function fromEntity(BaseEntity $deliveryOrderSettlementDetail)
    {
        $values = $deliveryOrderSettlementDetail->getAttributes();
        $this->setAttributes($values);

        $goods = GoodsRepository::repository()->findByPk($deliveryOrderSettlementDetail->goods_id);
    ...
    {
        $entity = DeliveryOrderSettlementDetail::create();
