...
use ddd\Profit\Domain\Price\IBuySettledPriceRepository;
use ddd\Profit\Domain\Service\EventService;
use ddd\infrastructure\error\ZException;
use ddd\infrastructure\error\ZModelSaveFalseException;

class BuySettledPriceRepository extends BaseRepository implements IBuySettledPriceRepository
{
    ...
    {
        if(empty($entity))
            throw new ZException("BuySettledPrice对象不存在");
        ...
        if(!empty($entity->bill_id))
            $model = \StockNotice::model()->with("priceDetails")->findByPk($entity->bill_id);

        if(!empty($model->priceDetails))
            return ;
            //throw new ZException("入库通知单编码".$model->code."的采购结算价格信息已经存在");
