...
use ddd\Profit\Domain\Price\ISellPriceRepository;
use ddd\Profit\Domain\Service\EventService;
use ddd\infrastructure\error\ZException;
use ddd\infrastructure\error\ZModelSaveFalseException;

class SellPriceRepository extends BaseRepository implements ISellPriceRepository
{
    ...
    {
        if(empty($entity))
            throw new ZException("SellPrice对象不存在");
        ...
        if(!empty($model->priceDetails))
            return ;
            //throw new ZException("合同编码".$model->contract_code."的销售价格信息已经存在");
