...
/**
 * Created by vector.
 * DateTime: 2017/10/24 11:20
 * Describe：进项票
 */
class InputInvoiceController extends AttachmentController
{
    public function pageInit()
    {
        $this->filterActions="getFile,reset,saveFile,delFile,getGoods,getPayment,submit,getTaxCode,getContractType,getCorpContracts,getCorpProjects,getCompany";
        $this->rightCode = "inputInvoice";
    ...
    public function actionIndex()
    {
        ...
        $sql    .= $query;
        $sql    .= " and a.type=".ConstantMap::INPUT_INVOICE_TYPE." and a.corporation_id in (".$user['corp_ids'].") order by a.apply_id desc {limit}";
    ...
    public function actionAdd()
    {
        ...
        if($data['type_sub']==ConstantMap::PAYMENT_GOODS_TYPE)
            $data['contract_type'] = ConstantMap::BUY_TYPE;

        $data['title_map_name'] = "invoice_input_type";
        ...
        $map = Map::$v;
        $title = $map['invoice_input_type'][$type];
    ...
    public function actionEdit()
    {
        ...
        $paymentItems   = InvoiceService::getPayment($data[0]['contract_id'], $id, 1);
        ...
        $data[0]['title_map_name'] = "invoice_input_type";
        // print_r($invoiceItems);die;
        $map = Map::$v;
        $title = $map['invoice_input_type'][$data[0]['type_sub']];
    ...
    public function actionDetail()
    {
        ...
        $plans          = InvoiceService::getPaymentById($id, 1);
    ...
    public function actionGetPayment()
    {
        ...
        $sql = "select plan_id,project_id,contract_id,pay_date,expense_type,expense_name,
                ...
                from t_payment_plan 
                where contract_id=".$id." and type=1 order by plan_id asc";
        $data    = Utility::query($sql);
    ...
    public function actionSave()
    {
        ...
        if (empty($obj->apply_id)){
            ...
            $obj->apply_code        = 'SP'.IDService::getId('invoice_SP', 5);
        ...
        try {
            $logRemark = ActionLog::getEditRemark($obj->isNewRecord, "货款进项票");
            $obj->save();
            ...
            if(empty($params['isSave'])){
                FlowService::startFlowForCheck15($obj->apply_id);

                TaskService::doneTask($obj->apply_id, Action::ACTION_30);
            }
    ...
    public function actionSubmit()
    {
        ...
        if(!$this->checkIsCanEdit($invoice->status))
            $this->returnError("当前状态下不可提交进项票申请信息！");
        ...
        try{
            ...
            FlowService::startFlowForCheck15($invoice->apply_id);

            TaskService::doneTask($invoice->apply_id, Action::ACTION_30);

            $trans->commit();
            Utility::addActionLog(json_encode(array("oldStatus"=>$oldStatus)), "提交货款进项票", "InvoiceApplication", $invoice->apply_id);
            $this->returnSuccess();
    ...
    public function actionGetTaxCode()
    {
        ...
        $invoice = InvoiceApplication::model()->findAllToArray(array("condition"=>"company_name='".trim($company_name)."'", "order"=>" apply_id desc"));
        if(Utility::isNotEmpty($invoice))
            $code = $invoice[0]['tax_code'];
    ...
    public function actionGetCorpContracts() {
        $corp_id = Mod::app()->request->getParam('corp_id');
        $contract_type = Mod::app()->request->getParam('contract_type');
        $project_id = Mod::app()->request->getParam('project_id');
        if (!Utility::checkQueryId($corp_id)) {
        ...
        $sql = 'select c.corporation_id,c.contract_id,c.contract_code
				from t_contract c
				left join t_project p on c.project_id=p.project_id 
				where c.status >= ' . Contract::STATUS_BUSINESS_CHECKED . ' and c.corporation_id = ' . $corp_id . $query . ' order by c.contract_id desc';

        $this->returnSuccess(Utility::query($sql));
    }

    public function actionGetCorpProjects() {
        $corp_id = Mod::app()->request->getParam('corp_id');
        $contract_type = Mod::app()->request->getParam('contract_type');
        ...
        $sql = 'select p.project_id, p.project_code, p.corporation_id, c.contract_id 
                from t_contract c 
                left join t_project p on p.project_id = c.project_id 
                where c.status >= ' . Contract::STATUS_BUSINESS_CHECKED . ' and c.corporation_id = ' . $corp_id . $query . $groupBy . ' order by p.project_id asc';
