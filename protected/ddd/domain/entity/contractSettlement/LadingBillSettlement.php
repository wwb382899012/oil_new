...
//        $entity->settle_date=Utility::getDate();
        ...
        if(is_array($ladingBill->items) && !empty($ladingBill->items)) {
            foreach ($ladingBill->items as $g)
            {
                ...
                $item->in_quantity = $g->in_quantity;
                ...
                if(!empty($g->in_quantity->unit) && $g->in_quantity->unit!=$g->in_quantity_sub->unit){
                    $flag = true;
                    $item->in_quantity_sub = $g->in_quantity_sub;
                    $item->settle_quantity_sub = $g->in_quantity_sub;
                    $item->loss_quantity_sub = new Quantity(0, $g->in_quantity_sub->unit);
                }
  
                $ladingItem = LadingBillSettlementItem::create($g->goods_id);
                $ladingItem->item_id = $item->item_id;
                $ladingItem->batch_id = $ladingBill->id;
                $ladingItem->in_quantity = $g->in_quantity;
                $ladingItem->settle_quantity = $g->in_quantity;
                $ladingItem->loss_quantity = new Quantity(0, $g->in_quantity->unit);
                if($flag) {
                    $ladingItem->in_quantity_sub = $g->in_quantity_sub;
                    $ladingItem->settle_quantity_sub = $g->in_quantity_sub;
                    $ladingItem->loss_quantity_sub = new Quantity(0, $g->in_quantity_sub->unit);
                }
