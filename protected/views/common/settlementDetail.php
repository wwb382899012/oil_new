...
<div class="box">
    <div class="box-header link with-border">
        <h3 class="box-title"><?php if($type==1) echo '入库通知单'; else echo '发货单'; ?>结算操作
            <?php if($type==1): ?>
                <a href="/stockIn/detail?id=<?php echo $bill_id;?>&t=1" target="blank"><?php echo $bill_code;?></a>
            <?php else:?>
                <a href="/deliveryOrder/detail?id=<?php echo $bill_id;?>&t=1" target="blank"><?php echo $bill_code;?></a>
            <?php endif;?>
        </h3>
    </div>
            ...
            <form class="col-sm-12">
                ...
                <div class="form-group">
                    <div class="col-sm-4">
                        ...
                        <p class="form-control-static col-sm-8"><?php echo $settlement["settle_date"]; ?></p>
                    </div>
                </div>
                <?php endif;?>
                <?php
                if(!empty($settlement['settlementGoods'])) {
                    foreach ($settlement['settlementGoods'] as $k => $v) { ?>
                        <fieldset
                            ...
                            <div class="clearfix">
                                <div class="pull-right">
                                    <?php if(!empty($v['hasDetail'])){ ?>
                                        <button type="button" class="btn btn-link hideBtn"  onclick="hideDetail(<?php echo $v['goods_id'].$bill_id ?>)"  id="<?php echo 'hideDetail_'.$v['goods_id'].$bill_id ?>"> 收起明细</button>
                                        <button type="button" class="btn btn-link showBtn" onclick="showDetail(<?php echo $v['goods_id'].$bill_id ?>)" id="<?php echo 'showDetail_'.$v['goods_id'].$bill_id ?>"> 展开明细</button>
                                    <?php } ?>
                                    <?php if($type==1){ ?>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4">
                                    <label class="col-sm-4 control-label"><?php if($type==1) echo '入'; else echo '出'; ?>库单数量 </label>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4">
                                    ...
                                    <p class="form-control-static col-sm-8"><?php echo $settlement['settle_currency']['ico'] . number_format($v['price']/100, 2); ?></p>
                                </div>
                                <div class="col-sm-4">
                                    <label class="col-sm-4 control-label">结算金额</label>
                                    <p class="form-control-static col-sm-8"><?php echo $settlement['settle_currency']['ico'] . number_format($v['amount']/100,2); ?></p>
                                </div>
                            ...
                            <?php } ?>
                            <?php if(!empty($v['hasDetail'])){ ?>
                                <fieldset id="<?php echo  'displayDetail_'.$v['goods_id'].$bill_id ?>">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        if(!empty($v['settleFiles'])){
                                            foreach ($v['settleFiles'] as $sf){
                                                echo '<p class="form-control-static col-sm-offset-4"><a href="/'.$controller.'/getFile/?id='.$sf['id'].'&fileName='.$sf['name'].'" target="_blank" class="btn btn-primary btn-xs">'.$sf['name'].'</a></p>';
                                            }
                                        }else{
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        if(!empty($v['goodsOtherFiles'])){
                                            foreach ($v['goodsOtherFiles'] as $of){
                                                echo '<p class="form-control-static col-sm-offset-4"><a href="/'.$controller.'/getFile/?id='.$of['id'].'&fileName='.$of['name'].'" target="_blank" class="btn btn-primary btn-xs">'.$of['name'].'</a></p>';
                                            }
                                        }else{
                                            echo '<p class="form-control-static col-sm-8">无</p>';
                                        }?>
        ...
        <div class="form-group">
            <div class="pull-right">
                <?php if(!empty($isCanEdit)){ ?>
                    <button type="button" class="btn btn-success" onclick="edit(<?php echo $bill_id?>)">修改</button>
                    <?php if(!empty($isCanSubmit)){?>
                        <button type="button" class="btn btn-danger" onclick="submit(<?php echo $bill_id?>)">提交</button>
                    <?php } ?>
                <?php } ?>
                <?php if(!$this->isExternal && empty($isHideBack)){ ?>
